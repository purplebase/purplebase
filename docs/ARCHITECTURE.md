...
RelaySubscription {
  ...
  eventCount: int  // Unique events received (after dedup)
...
print('Unique events: ${sub?.eventCount}');
...
| `maxRetries` | 31 | Before marking relay as failed |
| `maxLogEntries` | 200 | Rolling log size |

### Reconnection Backoff Schedule

Uses exponential backoff with repeated attempts at each level:
...
After 31 total attempts, the relay is marked as `failed`.
